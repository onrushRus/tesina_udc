 ...
 * @method     AporteQuery orderByTipoAporteId($order = Criteria::ASC) Order by the tipo_aporte_id column
 ...
 * @method     AporteQuery groupByTipoAporteId() Group by the tipo_aporte_id column
 ...
 *
 * @method     AporteQuery leftJoinTipoAporte($relationAlias = null) Adds a LEFT JOIN clause to the query using the TipoAporte relation
 * @method     AporteQuery rightJoinTipoAporte($relationAlias = null) Adds a RIGHT JOIN clause to the query using the TipoAporte relation
 * @method     AporteQuery innerJoinTipoAporte($relationAlias = null) Adds a INNER JOIN clause to the query using the TipoAporte relation
 *
 ...
 * @method     Aporte findOneByTipoAporteId(int $tipo_aporte_id) Return the first Aporte filtered by the tipo_aporte_id column
 ...
 * @method     array findByTipoAporteId(int $tipo_aporte_id) Return Aporte objects filtered by the tipo_aporte_id column
...
abstract class BaseAporteQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query on the tipo_aporte_id column
     *
     * Example usage:
     * <code>
     * $query->filterByTipoAporteId(1234); // WHERE tipo_aporte_id = 1234
     * $query->filterByTipoAporteId(array(12, 34)); // WHERE tipo_aporte_id IN (12, 34)
     * $query->filterByTipoAporteId(array('min' => 12)); // WHERE tipo_aporte_id > 12
     * </code>
     *
     * @see       filterByTipoAporte()
     *
     * @param     mixed $tipoAporteId The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return AporteQuery The current query, for fluid interface
     */
    public function filterByTipoAporteId($tipoAporteId = null, $comparison = null)
    {
        if (is_array($tipoAporteId)) {
            $useMinMax = false;
            if (isset($tipoAporteId['min'])) {
                $this->addUsingAlias(AportePeer::TIPO_APORTE_ID, $tipoAporteId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($tipoAporteId['max'])) {
                $this->addUsingAlias(AportePeer::TIPO_APORTE_ID, $tipoAporteId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(AportePeer::TIPO_APORTE_ID, $tipoAporteId, $comparison);
    }
    ...
    /**
     * Filter the query by a related TipoAporte object
     *
     * @param   TipoAporte|PropelObjectCollection $tipoAporte The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return   AporteQuery The current query, for fluid interface
     * @throws   PropelException - if the provided filter is invalid.
     */
    public function filterByTipoAporte($tipoAporte, $comparison = null)
    {
        if ($tipoAporte instanceof TipoAporte) {
            return $this
                ->addUsingAlias(AportePeer::TIPO_APORTE_ID, $tipoAporte->getIdTipoAporte(), $comparison);
        } elseif ($tipoAporte instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(AportePeer::TIPO_APORTE_ID, $tipoAporte->toKeyValue('PrimaryKey', 'IdTipoAporte'), $comparison);
        } else {
            throw new PropelException('filterByTipoAporte() only accepts arguments of type TipoAporte or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the TipoAporte relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return AporteQuery The current query, for fluid interface
     */
    public function joinTipoAporte($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('TipoAporte');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'TipoAporte');
        }
    ...
    /**
     * Use the TipoAporte relation TipoAporte object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   TipoAporteQuery A secondary query class using the current class as primary query
     */
    public function useTipoAporteQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinTipoAporte($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'TipoAporte', 'TipoAporteQuery');
    }
